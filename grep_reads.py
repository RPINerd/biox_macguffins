"""
    Grep Reads | RPINerd, 04/06/23

    RETIRED: My fastq filtering script (https://github.com/RPINerd/fastq_filtering)
    does basically the same thing but with more flexibility and in a neater way

    Given a regex (or file of expressions) to look for, grep_reads will
    find all reads that contain said pattern in the R1 fastq files provided.
    Then it will write these desired reads to a new fastq file and extract
    the matching paired reads from R2.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate a paired fastq file with a subset of reads matching a user-provided regex."
    )
    parser.add_argument("fastq", required=True, help="R1 fastq file to extract reads from")
    parser.add_argument(
        "filter", required=True, help="Either a regular expression of a file containing regular expressions"
    )
