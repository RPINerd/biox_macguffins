...
def cigar_filter(infile: str, filter_value: int = 50, outfile: str | None = None) -> list[list[str]]:
    """
    Barebones function to filter out reads based on a single CIGAR characteristic

    Args:
        infile (str): Path to the input SAM file
        filter_value (int): Minimum value for the CIGAR filter
        outfile (str): Path to the output file. If None, output is written to stdout
